...
def get_week_calendar(selected_year: int, selected_month: str):
    ...
    all_days_selected_month = pd.date_range(
        f"{selected_year}-{month_number}-01",
        periods=num_days,
        freq="D"
    )
    ...
    first_monday_after = get_monday_of_week(
        ...
        week_number=all_days_selected_month[0].isocalendar()[1]  # Get the ISO week number
    )
    # print(all_days_selected_month[0], first_monday_after, all_days_selected_month[0].isocalendar())

    first_sunday_before = get_sunday_of_week(
        year=all_days_selected_month[-1].isocalendar()[0],
        week_number=all_days_selected_month[-1].isocalendar()[1]  # Get the ISO week number
    )
    # print(all_days_selected_month[-1], first_sunday_before)

    all_days_selected_month = pd.date_range(first_monday_after, first_sunday_before, freq='d')

    # print(all_days_selected_month)
    month_df = pd.DataFrame(
        {"date": all_days_selected_month}
    )
    # print(month_df)
    ...
    activities = strava_manager.get_activities_between(
        start_date=first_monday_after,
        end_date=first_sunday_before
    )
    ...
    activities_df['start_date_local_str'] = activities_df['start_date_local'].dt.strftime('%Y-%m-%d')
    ...
    full_calendar['week_number'] = full_calendar['date'].dt.to_period('W').apply(lambda x: x.week)
    full_calendar['day'] = full_calendar['date'].dt.to_period('D').apply(lambda x: x.day)
    full_calendar['month'] = full_calendar['date'].dt.to_period('M').apply(lambda x: x.month)
    ...
    for day in ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]:
        calendar_day_head.append(
            html.Th(
                ...
                children=[
                    html.Span(className='xl:block lg:block md:block sm:block hidden', children=day),
                    html.Span(className='xl:hidden lg:hidden md:hidden sm:hidden block', children=day)
            ...
            children=[
                html.Table(
                    ...
                    children=[
                        html.Thead(
                            children=html.Tr(
                                children=calendar_day_head
                            )
                        ),
                        html.Tbody(
                            children=month_table_children
                        )
                    ]
                )
            ]
    ...
    calendar_container = [
        ...
        html.Button(f"""Back {session["selected_year"]}""", id={"type": "calendar-btn", "index": "back-year"}),
        week_calendar
    ]
...
def get_training_calendar(
    ...
    monthly_totals = activities_df[
        activities_df.year == year
        ].groupby(['year', 'month_of_year'])[['distance_km', 'moving_time']].sum().reset_index()

    all_months = {}
    # if we have value during this ti,e
    if len(monthly_totals) > 0:
        ...
        activities_dict = {
            pd.to_datetime(f"{year}-{month}-01").strftime('%b').upper(): {
                'distance': distance,
    ...
    for i in range(0, len(months), 4):
        ...
        for j in range(i, min(i + 4, len(months))):  # Adjusted the range to ensure it doesn't exceed the list length
            month_name = months[j]
            ...
            if month_name in all_months.keys():
                ...
                circle_size = normalize(distance_run, max_value)  # Adjust this based on your data
            ...
            square = html.Button(
                ...
                id={"type": "select-month-btn", "index": j},  # Use a unique ID for each button
                children=[
                    ...
                    html.Div(
                        ...
                        style={"width": f"{circle_size}px", "height": f"{circle_size}px"}
                    ),
                ],
            )
    ...
    year_selector = html.Div(
        ...
        children=[
            ...
            html.Div(className="month-name", children=f" {year} "),
